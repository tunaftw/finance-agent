...
def get_failed_files() -> set:
    """Hämta lista med misslyckade filer (för att hoppa över dem)."""
    try:
        data = json.loads(COMPLETION_LOG.read_text())
        failed = data.get("failed", [])
        # Hantera både gamla (strängar) och nya (dict) formatet
        return set(
...
def get_next_batch() -> list[Path]:
    ...
    skip = completed | failed  # Union: hoppa över både completed och failed
    batch = []
...
def show_progress():
    ...
    failed_set = get_failed_files()
    failed_count = len(failed_set)
    total = sum(1 for _ in TRANSCRIPT_QUEUE.read_text().splitlines() if _.strip())
    remaining = total - completed - failed_count

    pct = (completed / total * 100) if total > 0 else 0
    print(f"\n📊 Progress: {completed}/{total} ({pct:.1f}%) | ❌ Failed: {failed_count} | ⏳ Kvar: {remaining}")
